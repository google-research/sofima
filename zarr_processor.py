...
from sofima import coarse_registration, flow_utils, stitch_elastic, stitch_rigid, map_utils, mesh, zarr_utils
...
class ZarrFusion(warp.StitchAndRender3dTiles):
    ...
    def __init__(self, 
                 zarr_params: zarr_utils.ZarrDataset,
    ...
    def _open_tile_volume(self, tile_id: int):
        ...
        tile_volumes, tile_size_xyz = zarr_utils.load_zarr_data(self.zarr_params)
...
class ZarrStitcher: 
    ...
    def __init__(self,
                 input_zarr: zarr_utils.ZarrDataset,
        ...
        self.tile_volumes, self.tile_size_xyz = zarr_utils.load_zarr_data(input_zarr)
    ...
    def _run_fusion(self, 
                    output_cloud_storage: zarr_utils.CloudStorage,
                    output_bucket: str, 
                    output_path: str,
                    downsample_exp: int,
                    cx: np.ndarray, 
        ...
        if output_cloud_storage == zarr_utils.CloudStorage.S3:
        ...
        if downsample_exp != self.input_zarr.downsample_exp:
            ...
            fusion_zarr = zarr_utils.ZarrDataset(self.input_zarr.cloud_storage,
        ...
        # Perform fusion
        ds_out = zarr_utils.write_zarr(output_bucket, fused_shape_5d, output_path)
...
if __name__ == '__main__':
    ...
    cloud_storage = zarr_utils.CloudStorage.S3
    ...
    tile_layout = np.array([[1],
                            [0]])
    input_zarr = zarr_utils.ZarrDataset(cloud_storage=cloud_storage,
                                bucket=bucket,
    ...
    # Application Outputs
    output_cloud_storage = zarr_utils.CloudStorage.GCS
    output_bucket = 'YOUR-BUCKET-HERE'
    output_path = 'YOU-OUTPUT-NAME.zarr' 

    # Processing
    zarr_stitcher = ZarrStitcher(input_zarr, tile_layout)
    cx, cy, coarse_mesh = zarr_stitcher.run_coarse_registration()
    fine_mesh, fine_mesh_xy_to_index, stride_zyx = zarr_stitcher.run_fine_registration(cx, 
                                                                                       cy, 
                                                                                       coarse_mesh, 
                                                                                       stride_zyx=(20, 20, 20))
    zarr_stitcher._run_fusion(output_cloud_storage=output_cloud_storage,
